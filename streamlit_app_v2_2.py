...
st.set_page_config(page_title="WTC Intelligence Model V2.2", page_icon="🧠", layout="wide")

st.title("WiseTech Global — WTC Intelligence Model V2.2")
...
try:
    ...
    st.info(
        ...
        "Wait a minute and use **Rerun**. V2.2 caches successful downloads for 30 minutes."
    )
    st.stop()

features = usable_feature_columns(df)
features = clean_feature_list(df, features)
df = sanitize_ml_frame(df, features)
...
for tab, h in zip(tabs, HORIZONS):
    with tab:
        imp = (
            pd.DataFrame({
                "Feature": models[h].get("features", features),
...
with st.expander("V2.2 deployment fixes"):
    st.markdown("""
...
- **New in V2.2:** +/- infinity and float32-overflow values are sanitized before training and prediction.
""")
